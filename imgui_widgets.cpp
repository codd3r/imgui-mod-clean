...
#include <examples/example_win32_directx9/options.hpp>
...
bool ImGui::tab(const char* icon, const char* text, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (selected) {
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(60, 61, 81, int(255 * GetStyle().Alpha)), 5);
    }
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5 + 27, bb.Min.y + 7), ImColor(96, 96, 109, int(255 * GetStyle().Alpha)), text);

    if (selected)
    {
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5 + 27, bb.Min.y + 7), ImColor(178, 182, 255, int(255 * it_filled->second)), text);
    }

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5 + 7, bb.Min.y + 7), ImColor(96, 96, 109, int(255 * GetStyle().Alpha)), icon);

    if (selected)
    {
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5 + 7, bb.Min.y + 7), ImColor(178, 182, 255, int(255 * it_filled->second)), icon);
    }
...
bool ImGui::SubTabEx(const char* label, bool* active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_Text, *active ? ImVec4(178 / 255.f, 182 / 255.f, 255 / 255.f, 1.f) : ImVec4(135 / 255.f, 135 / 255.f, 142 / 255.f, 1.f));
...
bool ImGui::renderer_tab(const char* icon, const char* text, bool selected)
{
    ...
    if (selected) {
        ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + 130, bb.Min.y + 36 - 7), ImColor(25, 24, 25), 5);
    }

    ImGui::PushFont(opt.InterMedium);
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 10 + 26, bb.Min.y + 12 - 6), selected ? ImColor(255, 255, 255) : ImColor(135, 133, 135), text);
    ImGui::PopFont();

    ImGui::PushFont(opt.IconFont);
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 10, bb.Min.y + 9 - 6 + 2), selected ? ImColor(255, 255, 255) : ImColor(135, 133, 135), icon);
    ImGui::PopFont();
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(ImVec2(window->DC.CursorPos.x + 18 + 200 + 25 - 6 - 7 - 14 + 4, window->DC.CursorPos.y + 4.5f - 7), window->DC.CursorPos + ImVec2(18 + 200 + 12 - 3 + 7 - 14 + 4, -5.5f + 7) + ImVec2(label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2));
    ...
    if (*v) {
        ImGui::GetWindowDrawList()->AddRectFilledMultiColor_Rounded(check_bb.Min - ImVec2(1 - 7, 1 - 7), check_bb.Max + ImVec2(2 - 7, 2 - 7), ImColor(142, 124, 204), ImColor(112, 113, 199), ImColor(81, 100, 203), ImColor(111, 110, 204), 2.5f);
        RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 200 - 1, (total_bb.Min.y + total_bb.Max.y) / 2 - 5 + 1), ImColor(255, 255, 255), 8);
    }
    else {
        RenderFrame(check_bb.Min - ImVec2(1 - 7, 1 - 7), check_bb.Max + ImVec2(2 - 7, 2 - 7), GetColorU32(ImVec4(26 / 255.f, 25 / 255.f, 27 / 255.f, 255 / 255.f)), true, 2.5f, false, ImColor(37, 36, 37));
        if (hovered) {
            RenderCheckMark(window->DrawList, ImVec2(total_bb.Min.x + 200 - 1, (total_bb.Min.y + total_bb.Max.y) / 2 - 5 + 1), ImColor(124, 124, 124), 8);
        }
    }

    ImGui::PushStyleColor(ImGuiCol_Text, *v ? ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f) : ImVec4(95 / 255.f, 95 / 255.f, 95 / 255.f, 1.f));

    if (label_size.x > 0.0f)
        RenderText(text_bb.GetTL() - ImVec2(17, 0), label);
    ImGui::PopStyleColor();

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));

    ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 31 + 5 + 2 - 1);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float w = CalcItemWidth();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(154, -10) - ImVec2(10, 0), window->DC.CursorPos + ImVec2(window->Size.x, 20 + label_size.y) - ImVec2(50, 0) - ImVec2(10, 0));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ImRect grab_bb;
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + 32), ImVec2(frame_bb.Max.x - 5, frame_bb.Max.y - 7), ImColor(43, 43, 49, int(255 * GetStyle().Alpha)), 3);

    //line
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + 32), ImVec2(grab_bb.Max.x - 5, grab_bb.Max.y - 5), ImColor(178, 182, 255, int(255 * GetStyle().Alpha)), 3);
    }

    RenderFrame(grab_bb.Max - ImVec2(14 , 9) - ImVec2(0, 9), grab_bb.Max + ImVec2(4, 8) - ImVec2(0, 8), GetColorU32(ImVec4(43 / 255.f, 43 / 255.f, 49 / 255.f, 255 / 255.f)), true, 2.5f, false, ImColor(178, 182, 255));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 170 - 18 + 8, frame_bb.Min.y + 32 - 4), ImVec2(frame_bb.Max.x + 50 - 7, frame_bb.Max.y - 7 + 4), ImColor(43, 43, 49, int(255 * GetStyle().Alpha)), 3);

    auto label_buffer_size = CalcTextSize(value_buf, value_buf_end, true);
    PushStyleColor(ImGuiCol_Text, ImColor(178, 182, 255, int(255 * GetStyle().Alpha)).operator ImVec4());
    RenderText(ImVec2(frame_bb.Max.x - label_buffer_size.x + 35 + 2, (frame_bb.Min.y + frame_bb.Max.y) / 2 + 3 + 1), value_buf, value_buf_end);
    PopStyleColor();
    PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * GetStyle().Alpha)).operator ImVec4());
    RenderText(ImVec2(frame_bb.Min.x - 137 + 10, (frame_bb.Min.y + frame_bb.Max.y) / 2) + 2, label);
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() + ImVec2(0, border ? 20 : 5), ImGui::GetWindowPos() + size_arg, ImColor(21, 19, 21), 5); //bg
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() + ImVec2(1, border ? 20 : 5 +1), ImGui::GetWindowPos() + size_arg - ImVec2(1,1), ImColor(34, 32, 34), 5);//обводка
    parent_window->DrawList->AddLine(ImGui::GetWindowPos() + ImVec2(0, 24 + 3), ImGui::GetWindowPos() + ImVec2(size_arg.x, 24 + 4 + 3), ImColor(34, 32, 34)); //seperator
    parent_window->DrawList->AddText(ImGui::GetWindowPos() + ImVec2(115, 8), ImColor(255, 255, 255), name);
